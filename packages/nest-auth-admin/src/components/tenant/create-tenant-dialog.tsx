import React, { useEffect } from 'react';
import { Building2 } from 'lucide-react';
import Icon from '@mui/material/Icon';
import Alert from '@mui/material/Alert';
import Stack from '@mui/material/Stack';
import { useForm } from 'react-hook-form';
import { yupResolver } from '@hookform/resolvers/yup';
import type { SubmitErrorHandler } from 'react-hook-form';
import { FormDialog } from '../form-dialog';
import { RHFTextField } from '../form/hook-form-fields/rhf-text-field';
import { tenantEmptyValues, tenantSchema, type TenantFormData } from './tenant-form-schema';

export interface CreateTenantDialogProps {
    open: boolean;
    onClose: () => void;
    onSubmit: (data: TenantFormData) => Promise<void>;
    error?: string;
    onError?: SubmitErrorHandler<TenantFormData>;
}

export const CreateTenantDialog: React.FC<CreateTenantDialogProps> = ({
    open,
    onClose,
    onSubmit,
    error,
    onError,
}) => {
    const methods = useForm<TenantFormData, unknown, TenantFormData>({
        resolver: yupResolver(tenantSchema) as any,
        defaultValues: tenantEmptyValues,
    });

    const { reset } = methods;

    useEffect(() => {
        if (open) {
            reset(tenantEmptyValues);
        }
    }, [open, reset]);

    const handleSuccess = async (data: TenantFormData) => {
        await onSubmit(data);
    };

    return (
        <FormDialog
            formContext={methods}
            onSuccess={handleSuccess}
            onError={onError}
            open={open}
            onClose={onClose}
            title="Create New Tenant"
            subTitle="Add a new tenant workspace"
            icon={<Icon component={Building2} sx={{ color: 'primary.main' }} />}
            maxWidth="md"
        >
            <Stack spacing={2} sx={{ p: 2 }}>
                {error && (
                    <Alert severity="error" sx={{ '& .MuiAlert-message': { width: '100%' } }}>
                        {error}
                    </Alert>
                )}
                <RHFTextField name="name" id="tenant-name" label="Tenant Name" placeholder="Acme Corporation" required fullWidth />
                <RHFTextField
                    name="slug"
                    id="tenant-slug"
                    label="Slug"
                    placeholder="acme-corp"
                    helperText="URL-friendly identifier (lowercase, hyphens only)"
                    required
                    fullWidth
                />
                <RHFTextField
                    name="description"
                    id="tenant-description"
                    label="Description"
                    placeholder="Brief description of this tenant..."
                    multiline
                    minRows={2}
                    fullWidth
                />
            </Stack>
        </FormDialog>
    );
};
