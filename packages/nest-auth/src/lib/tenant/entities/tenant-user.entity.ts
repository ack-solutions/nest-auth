...
@Entity('nest_auth_tenant_users')
@Unique(['userId', 'tenantId'])
export class NestAuthTenantUser extends BaseEntity {
    @PrimaryGeneratedColumn('uuid')
    id: string;

    @Column({ nullable: true })
    @Index()
    @RelationId((membership: NestAuthTenantUser) => membership.user)
    userId: string;

    @Column({ nullable: true })
    @Index()
    @RelationId((membership: NestAuthTenantUser) => membership.tenant)
    tenantId: string;
    ...
    @JoinTable({
        name: 'nest_auth_tenant_user_roles',
        joinColumn: { name: 'nestAuthTenantUserId', referencedColumnName: 'id' },
