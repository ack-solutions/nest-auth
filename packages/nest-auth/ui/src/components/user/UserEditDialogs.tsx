...
export const EditRolesModal: React.FC<EditModalProps & { roles: Role[]; tenants: Tenant[] }> = ({
    ...
    useEffect(() => {
        if (isOpen && user.tenantMemberships?.length) {
            const next: Record<string, string[]> = {};
            for (const m of user.tenantMemberships) {
                next[m.tenantId] = m.roleIds ?? [];
            }
    ...
    const membershipTenants = (user.tenantMemberships ?? []).map((m) => ({
        tenantId: m.tenantId,
