...
Exempt a route (or whole controller) from the "must change password" hard-block, so a user with `mustChangePassword: true` can still reach it. Only matters when `mustChangePassword.enforce: true` is set in the module config (see [NestAuthModule](/docs/backend/module)).

```ts
@SkipMustChangePassword()
@Auth()
@Get('/billing/invoices')
invoices() { /* reachable even while a password change is pending */ }
```

The library already marks its own change-password, logout, current-user/session, verification, and MFA routes — add this to your own routes that must stay reachable while a change is pending.
...
| Decorator | Returns |
|---|---|
| `@CurrentTenantId()` | `string \| null` — active tenant ID (`null` when none is resolved; handlers must handle it) |
| `@CurrentTenant()` | **Deprecated** — alias of `@CurrentTenantId()`, also returns `string \| null` (not a tenant entity). Slated for removal in v3 |
