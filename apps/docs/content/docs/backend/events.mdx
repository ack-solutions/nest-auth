...
| `UserInvitedEvent` | `NestAuthEvents.USER_INVITED` | `user`, `tenantId`, `token` (single-use set-password token — for the invite email; never logged/returned), `isNewUser`, `invitedBy`, `metadata` |
...
Emitted by `RoleService` / `PermissionService` on every mutation, so you can sync
RBAC changes to an external system or audit log.

| Event | Constant | Payload |
|---|---|---|
| `RoleCreatedEvent` | `NestAuthEvents.ROLE_CREATED` | `role` |
| `RoleUpdatedEvent` | `NestAuthEvents.ROLE_UPDATED` | `role`, `updatedFields` |
| `RoleDeletedEvent` | `NestAuthEvents.ROLE_DELETED` | `role` |
| `PermissionCreatedEvent` | `NestAuthEvents.PERMISSION_CREATED` | `permission` |
| `PermissionUpdatedEvent` | `NestAuthEvents.PERMISSION_UPDATED` | `permission`, `updatedFields` |
| `PermissionDeletedEvent` | `NestAuthEvents.PERMISSION_DELETED` | `permission` |
