...
The context value exposes `accounts`, `activeAccount`, `activeAccountId`, and the actions `addAccount`, `switchAccount`, `removeAccount`, plus:

- `completeMfa(error, verifyDto, options?)` — finish an `addAccount` that threw `AccountMfaRequiredError`: it verifies the challenge on the pending client and registers the account.
- `setAccountMeta(accountId, meta)` — set an account's display `{ label, tenantName }` for the switcher UI.
- `manager` — the underlying `IAccountSwitcher`, an escape hatch for advanced flows (e.g. `manager.commitAccount`).

See [Multi-account hooks](/docs/react/hooks#multi-account-hooks) for the full destructure and the MFA-on-add flow.
