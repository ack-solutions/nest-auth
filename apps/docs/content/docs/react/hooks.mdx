...
| `mustChangePassword` | `boolean?` | Set on the snapshot returned by `addAccount` / `completeMfa` when the member is on an admin-issued temporary password. **One-shot** — never on `accounts[]` (see below) |
...
An admin-issued temporary password sets `mustChangePassword` on the member. The sign-in call reports it, so a multi-account app can route to its change-password screen without a follow-up `/auth/me`:

```tsx
const account = await addAccount(loginDto);
if (account.mustChangePassword) router.push('/change-password');
```

> **One-shot, on purpose.** It is only on the snapshot returned by `addAccount` / `completeMfa` — **not** on `accounts[]` or `activeAccount`, and it is not persisted. A cached `true` would outlive the password change and bounce the user back to the change-password screen forever. Re-check later with `getSessionUserData()` (`GET /auth/me`), which is always current; `undefined` means "not observed here", not "false". See [Force password change](/docs/recipes/force-password-change).
