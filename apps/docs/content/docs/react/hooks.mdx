...
```tsx
const {
  accounts,        // AccountSnapshot[] — { accountId, email, label, isActive }
  activeAccount,   // the active one, or null
  addAccount,      // (loginDto) => Promise — log in a new account, keep the others
  switchAccount,   // (accountId) => Promise — repoint active (no server call)
  removeAccount,   // (accountId) => Promise — sign that account out
} = useAccountSwitcher();
