---
title: Upgrading within v2
description: Upgrading within the v2 line — additive/non-breaking minors, plus the 2.8.0 security release which turns on a few fail-closed defaults.
---
...
> Upgrading across the **v1 → v2** major instead? That's a separate, larger jump — see the [v1 → v2 migration guide](/docs/migration).
...
| want a **multi-account** account switcher | **`2.1.0`** — opt in with `session.allowMultipleAccounts`, then use `AccountManager` / `AccountSwitcherProvider`. Off by default. See [Multi-account](/docs/recipes/multi-account-switching). |
...
- [v1 → v2 migration guide](/docs/migration) — the major-version upgrade.
