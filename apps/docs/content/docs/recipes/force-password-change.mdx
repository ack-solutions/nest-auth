...
```ts
const account = await accounts.addAccount(loginDto);
if (account.mustChangePassword) routeTo('/change-password');
```
...
> **It is one-shot, on purpose.** `mustChangePassword` appears only on the snapshot returned by the **sign-in call** — never on `listAccounts()` snapshots, and it is not persisted. A cached `true` would survive the password change and bounce the user back to the change-password screen forever. To re-check later, read `/auth/me` (`client.getSessionUserData()`), which is always current. `undefined` means "not observed here", not "false" — with `enforce: true` the guard is still the real enforcement, so a missed redirect degrades to a server-side block, never to open access.

## Related

- [Invite a member](/docs/recipes/tenant-login-isolated#4-invite-a-member-first-class-since-250).
- [Multi-account login & switching](/docs/recipes/multi-account-switching) — the `addAccount` sign-in path.
