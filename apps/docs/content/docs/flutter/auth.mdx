...
Apple returns the user's name only on the **first** sign-in — pass `givenName`/`familyName` so the backend can persist it (they're forwarded to the API as `firstName`/`lastName`). `nonce` is optional replay protection. Apple provides no avatar.
...
// firstName/lastName are needed for Apple; avatarUrl is optional (Apple has none).
await auth.socialLogin('apple', identityToken, nonce: nonce, firstName: 'Ada', lastName: 'Lovelace');
```
