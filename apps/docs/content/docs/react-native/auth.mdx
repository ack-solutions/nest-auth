...
Acquire the provider token with a native SDK (for example `@react-native-google-signin/google-signin` or Apple Authentication), then hand it to the underlying client. Reach the client via `client` from `useNestAuth()`:
