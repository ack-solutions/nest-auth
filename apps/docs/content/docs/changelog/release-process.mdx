...
The docs site's GitHub Actions workflow ([`.github/workflows/docs.yml`](https://github.com/ack-solutions/nest-auth/blob/main/.github/workflows/docs.yml)) builds on every push to `main` that touches `apps/docs/**` or any of the four packages. It also re-runs the auto-generation pipeline (TypeDoc + OpenAPI + SQL snapshots), so the `API Reference` and `Database Setup` pages stay in sync with the latest code.
...
1. Add a CNAME record at your DNS provider pointing `docs.ackplus.com` → `ack-solutions.github.io`.
2. In **Settings → Pages → Custom domain**, enter `docs.ackplus.com` and save.
3. Set the env var `GITHUB_PAGES_USER_SITE=true` on the docs build step in [`docs.yml`](https://github.com/ack-solutions/nest-auth/blob/main/.github/workflows/docs.yml) — custom-domain Pages serves from the root, not a subpath, and the basePath logic in `next.config.mjs` keys off this flag.
