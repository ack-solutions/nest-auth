...
`nest_auth_flutter` on pub.dev. **Current stable: `2.8.0`.**
...
The P0 security-hardening release: ~24 fixes across the auth core and the embedded admin console. A handful **change how existing apps boot or authenticate** — read the **Breaking** items and the **[2.7.x → 2.8.0 upgrade steps](/docs/changelog/migrations#2-7-x--2-8-0--security-hardening)** before you bump. Everything under **Added (opt-in)** is off by default, so nothing else changes until you enable it.
